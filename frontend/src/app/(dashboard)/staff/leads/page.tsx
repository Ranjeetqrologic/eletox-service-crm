...
const statuses = ["started", "working", "need_parts", "pending", "follow_up", "completed", "cancelled"];
...
export default function StaffLeads() {
  ...
  const completeJob = async (jobId: string) => {
    try {
      await api.put(`/jobs/${jobId}/report`, { ...report, status: "completed" });
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-auto">
            ...
            <div className="space-y-3">
              ...
              <input type="number" placeholder="Bill Amount" className="border p-2 rounded w-full" onChange={(e) => setReport({ ...report, billAmount: Number(e.target.value) })} />
              <input type="number" placeholder="Received Amount" className="border p-2 rounded w-full" onChange={(e) => setReport({ ...report, receivedAmount: Number(e.target.value) })} />
              ...
              <input type="number" placeholder="Rating 1-5" min="1" max="5" className="border p-2 rounded w-full" onChange={(e) => setReport({ ...report, rating: Number(e.target.value) })} />
            </div>
            <div className="mt-4 flex gap-3">
              <button onClick={() => completeJob(selected._id)} className="bg-green-600 text-white px-4 py-2 rounded">Complete Job</button>
